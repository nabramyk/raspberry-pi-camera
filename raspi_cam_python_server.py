...
def camera_grab(pid, cr, parameters, camera_counter):
	...
	if timelapse_running=="true":
		...
		while cr.value:
			...
			output = subprocess.run(params + [image_directory + output_session_name + temp + parse_time_replacement_characters(output_filename) + '.' + output_format], stdout=subprocess.PIPE)
		...
		output = subprocess.run(params + [image_directory + parse_time_replacement_characters(output_filename) + '.' + output_format], stdout=subprocess.PIPE)
